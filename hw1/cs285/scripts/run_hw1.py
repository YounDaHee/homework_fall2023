...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        # 여기서 환경에서 수행할 policy 정의.
        # BC에서는 그냥 expert.
        # DAgger 에서는 새로운 pi = (bata)pi_hat + (1-beta)pi*
        ...
        if itr == 0:
            ...
            assert params['do_dagger'] # BC에서는 itr이 2 이상이 되면 안된다. 
            ...
            if params['do_dagger']:
                ...
                for p in range(len(paths)) :
                    for selec in range(len(paths[p]['action'])) :
                        if(np.random.rand(1)<beta) :
                            selec_obs = torch.from_numpy(paths[p]['observation'][selec]).float().to(ptu.device)
                            replace_action = expert_policy(selec_obs)
                            paths[p]['action'][selec] = replace_action.detach().cpu().numpy()
                            cnt += 1

                print(f"{cnt} learner action replaced by expert")
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          train_set = np.random.choice(range(len(replay_buffer.obs)), params['train_batch_size'], replace=False)

          ob_batch = replay_buffer.obs[train_set]
          ac_batch = replay_buffer.acs[train_set]
        ...
        plt.plot([float(log['Training Loss']) for log in training_logs], marker='o', linestyle='-', label='Training Loss')
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f'training_loss{itr}.png')  # 이미지로 저장
        plt.close()
